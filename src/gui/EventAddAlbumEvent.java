...
import datas.AlbumPhotoEvent;
import datas.Event;
import datas.Personne;
import controlers.PicView;

public class EventAddAlbumEvent extends JDialog {
	 ...
	 * @param modal
	 */
	public EventAddAlbumEvent(WindowFrame parent, String title, boolean modal, PicView pcw){
	...
	private class Button implements ActionListener{
		public void actionPerformed(ActionEvent arg0) {
			Event evt = pcw.getEvent(jl.getSelectedValue());
			if(!pcw.eventIsEmpty())
				pcw.ajouterAlbum(new AlbumPhotoEvent(evt.getNom(), evt));
			EventAddAlbumEvent.this.dispose();
		}
	}
}